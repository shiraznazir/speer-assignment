...
const Header = () => {
  ...
  return (
    <header className="grid grid-flow-col bg-white w-full md:w-6/12 fixed z-50">
      <div className="ml-2 flex relative">
        ...
        <h1 className="absolute top-1/4 left-8 my-auto font-bold">Activity</h1>
        <div className="absolute bottom-0 right-0 bg-gray-100 w-2/12 h-1/4"></div>
        <div className="absolute bottom-[9px] right-[-10px] arrow-right"></div>
        <div className="absolute bottom-0 right-4 bg-white w-2/12 h-2/4 rounded-ee-3xl"></div>
      </div>

      <div className="flex mt-1 bg-gray-100 rounded-tl-3xl justify-evenly">
        <div
          className="flex flex-col cursor-pointer"
          onClick={() => setActivity(0)}
        >
          <h6
            className={`mt-4 font-medium mx-2 text-sm text-${
              activity === 0 ? "gray-600" : "[#A6A6A6]"
            }`}
          >
            Inbox
          </h6>
          {activity === 0 && <div className="h-4 red-line"></div>}
        </div>
        <OptionIcon className="my-auto" />
        <div
          className="flex flex-col cursor-pointer"
          onClick={() => setActivity(1)}
        >
          <h6
            className={`mt-4 font-medium text-${
              activity === 1 ? "gray-600" : "[#A6A6A6]"
            } mx-2 text-sm`}
          >
            Archive
          </h6>
          {activity === 1 && <div className="h-4 red-line"></div>}
        </div>
        <OptionIcon className="my-auto" />
